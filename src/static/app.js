  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Participants section markup with delete icon, no bullet
        const participantsMarkup = details.participants && details.participants.length
          ? `<ul class="participants-list no-bullets">
               ${details.participants.map((participant) => `
                 <li class="participant-row">
                   <span class="participant-name">${participant}</span>
                   <button class="delete-participant" title="Remove participant" data-activity="${encodeURIComponent(name)}" data-participant="${encodeURIComponent(participant)}">🗑️</button>
                 </li>
               `).join("")}
             </ul>`
